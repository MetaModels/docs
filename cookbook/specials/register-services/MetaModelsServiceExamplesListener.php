...
final readonly class MetaModelsServiceExamplesListener
{
    ...
    public function onMetaModelsServiceExamples(PrePersistModelEvent $event): void
    {
        ...
        // Security - more explanations in the following text section.
        // Recommended: security.helper - can check permissions AND fetch the user.
        ...
        // Reload the Contao MemberModel (with all DB fields) by username.
        $member = MemberModel::findByUsername($user->getUserIdentifier());

        // Alternative 1: Contao framework (legacy). getInstance() returns the current
        // frontend user as a Contao object including the DB fields - createInstance()
        // would be a new, empty instance and therefore wrong.
        $member = $this->framework->getAdapter(FrontendUser::class)->getInstance();

        // Alternative 2: security.token_storage (low-level, no permission check).
